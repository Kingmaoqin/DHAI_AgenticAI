...
from agent.common import FWAEval, get_fwa_green_agent_card
import agent.metrics.automatic.automatic_evaluation as auto_eval
from agent.metrics.tasks import (
    BenchmarkDataSource,
    TaskLoader,
    build_goal,
)
from agent_core.green_executor import GreenAgent, GreenExecutor
from agent_core.models import EvalRequest, EvalResult
from agent_core.purple_client import PurpleClient
from log.fwa_logger import getLogger, set_logger
...
class FWAGreenAgent(GreenAgent):
    ...
    def validate_request(self, request: EvalRequest) -> tuple[bool, str]:
        if 1:
            self._data_source = BenchmarkDataSource(access_token="")
            return True, "ok"
    ...
    async def run_eval(self, req: EvalRequest, updater: TaskUpdater) -> None:
        ...
        try:
            # Load task from json
            tasks = TaskLoader()
            tasks = tasks.extract_tasks('custom')
            ...
            for task in tasks:
                try:
                    ...
                    if 1:
                        if 1:
                            path_tmp = ""
                            if type(task["input_data"]) is list:
                                path_tmp = task["input_data"][0]
                            elif type(task["input_data"]) is str:
                                path_tmp = task["input_data"]
                        from pathlib import Path
                        file_path_obj = Path(path_tmp)
                        ...
                        repo_path = f"TestingDataset/{subdirectory}/{file_name}"
                    if 1:
                            local_path = repo_path
                            logger.info(f"Processing data information: {task['input_data']}")
                            base64_data = self._data_source._load_base64(Path(local_path))
                            media_type  = self._data_source._get_media_type(Path(local_path))

                            file_payloads = [FileWithBytes(
                                bytes=base64_data,
                    ...
                    result = await self.orchestrate({'agent': 'http://127.0.0.1:9019/'}, goal, file_payloads, updater)
